...
namespace ka::p2p {
    ...
    bool configure_gate_loop(pn532::controller &ctrl, keymaker &km, std::string gate_description) {
    ...
    pn532::p2p::result<> configure_gate_exchange(keymaker &km, secure_initiator &comm, std::string gate_description) {
        ...
        km.register_gate(gid, comm.peer_pub_key(), std::move(gate_description));
        return mlab::result_success;
    }
    ...
    bool configure_gate_in_rf(pn532::controller &ctrl, std::uint8_t logical_index, keymaker &km, std::string gate_description) {
        pn532::p2p::pn532_initiator raw_comm{ctrl, logical_index};
        secure_initiator comm{raw_comm, km.keys()};
        return bool(configure_gate_exchange(km, comm, std::move(gate_description)));
    }
}